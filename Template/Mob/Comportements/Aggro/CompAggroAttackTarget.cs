...
public class CompAggroAttackTarget: CompAggroGoTarget{
        ...
        public override bool AggroAction(MobTargets targets,float delta){       
                ...
                if(IsAttacking) {
                        Attacks[AttackIndex].AttackAction(delta);
                        return false;
                }
                targets.Reset();
                ...
                ////////////////////
                Player target = SelectTarget.SelectTarget(targets,Attacks);
                AttackIndex = SelectAttack.SelectAttack(targets,Attacks);//TODO don't use index but IMobAttack ? have to save so ?
                IsAttacking = true;
                Attacks[AttackIndex].AddFinishAction(AttackFinish);
                Attacks[AttackIndex].Start(Parent,target);
